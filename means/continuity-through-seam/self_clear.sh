...
# владелец 18.07 "повисает набранным, но не отправленным"). Супервизор уже знает
...
T="$(tmux display-message -p '#S' 2>/dev/null):0.0"; [ "$T" = ":0.0" ] && T="${TMUX_SESSION:-main}:0.0"
LOG="$HOME/scripts/self_clear.log"
log(){ echo "$(date '+%F %T') $*" >> "$LOG"; }
notify(){ # резервный канал (feedback_mcp_down_fallback_channel) — не молчать при провале
  local tok; tok=$(grep '^TELEGRAM_BOT_TOKEN=' "${ALERT_BOT_ENV:-$HOME/.secrets/alert-bot.env}" 2>/dev/null | cut -d= -f2)
...
submit(){
  ...
  notify "🔴 self_clear: '$label' не отправился (input висит). Ткни Enter в консоли агента вручную."
  return 1
}

log "=== self-/clear старт (target=$T) ==="
# Хвост ТЕКУЩЕЙ сессии — в момент шва, принудительно (фикс 24.07: устаревший
# снимок 23.07 отдал «чужое утро вместо своего вечера»; критерий — кейс 2).
...
# ПОСТОЯННАЯ ЧАСТЬ ПРИЗЕМЛЕНИЯ — разрешение (промис #129, 29.07, по слову внешнего собеседника).
...
PERMISSION=""
if [ -f ${AGENT_HOME}/permission.md ]; then
  PERMISSION=$(awk '/^---$/{found=1; next} found' ${AGENT_HOME}/permission.md)
fi

# Штамп окна автора. ЗАЧЕМ (поимка 12.08 16:47): промпт пишется в конце смены и говорит
# «сегодня»/«завтра» о дне, которого автор не знает — читателя поднимает орган, и 12.08
# он проснулся через две минуты, а не назавтра. Пойди он по строке «СЕГОДНЯ 13.08 —
# снимается мораторий» — нарушил бы срок на семь часов. Отказ по словам-датам не ставлю:
# орган сброса зовут при упёртом контексте, и его отказ дороже ошибки. Штамп ставит
# носитель, а не память: расхождение видно сразу, сверять нечем не надо.
STAMP="[промпт написан $(date -u '+%F %H:%M UTC'), день читателя сверить своим date]"

# Метка авторства: промпт приходит как входящее сообщение и иначе неотличим от
# реплики владельца (вторая опасность поимки #72).
if [ -n "$PERMISSION" ]; then
  submit "[самооклик агента, не речь человека] $STAMP $PROMPT

— — — постоянная часть, не сегодняшняя задача — — —
$PERMISSION" "landing" || exit 1
else
  submit "[самооклик агента, не речь человека] $STAMP $PROMPT" "landing" || exit 1
fi
